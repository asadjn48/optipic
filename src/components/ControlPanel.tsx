...
export const ControlPanel: React.FC<ControlPanelProps> = ({
  ...
  return (
// Inside ControlPanel.tsx return statement:

<div className="space-y-6 p-5 rounded-3xl bg-[#12141D] border border-[#24283B] shadow-xl shrink-0">
  {/* ... target header ... */}
  ...
  <div className="relative">
    <input
      ...
      className="w-full bg-[#0B0C10] border border-[#24283B] rounded-xl px-4 py-3 text-slate-100 placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all font-medium"
      placeholder="e.g., 150"
    />
    <div className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 font-mono text-xs uppercase font-bold bg-[#12141D] px-2 py-1 rounded-md border border-[#24283B]">
      KB
    </div>
  </div>
    ...
    {QUICK_SIZES.map((size) => (
      <button
        ...
        className={cn(
          ...
          targetSize === size
            ? "bg-indigo-500 text-white shadow-lg shadow-indigo-500/25 ring-2 ring-indigo-500/50 ring-offset-2 ring-offset-[#12141D]"
            : "bg-[#1A1D2A] text-slate-400 border border-[#24283B] hover:bg-[#24283B] hover:text-slate-200"
        )}
      >
        {size >= 1000 ? `${size / 1000} MB` : `${size} KB`}
      </button>
      ...
      <button
        ...
        disabled={disabled || isCompressing}
        className={cn(
          "w-full relative overflow-hidden rounded-xl py-3.5 font-bold transition-all duration-300 flex justify-center",
          isCompressing
            ? "bg-indigo-600/50 text-white cursor-wait"
            : disabled
              ? "bg-zinc-800/50 text-zinc-500 cursor-not-allowed border border-zinc-800"
      ...
      >
        <div className="relative z-10 flex items-center justify-center gap-2">
          {isCompressing ? (
            ...
            <Wand2 className={cn("w-4 h-4", disabled ? "opacity-50" : "")} />
          )}
          <span>
            {isCompressing 
              ? 'Processing Batch...' 
              : disabled 
                ? 'Add images to begin' 
                : `Compress ${fileCount} Image${fileCount > 1 ? 's' : ''}`}
          </span>
        </div>
      </button>
    </div>
